...
import { CardThumbnailTwoCode } from "data/code/components-code/CardCode";
import ProductCardThree from "components/cards/ProductCardThree";

const CardThumbnailTwo = () => {
  return (
    <Row>
      <Col xl={12} lg={12} md={12} sm={12} xs={12}>
        <div className="mb-4">
          <h2 className="mb-0">Card Thumbnail #2</h2>
        </div>
        <div className="mb-10">
            ...
            >
              <Col xl={8} lg={8} md={12} xs={12} className="mx-auto">
                {products.slice(4, 5).map((item) => (
                  <ProductCardThree product={item} key={item.id} />
                ))}
              </Col>
            </Tab.Pane>
            <Tab.Pane title="Code" eventKey={"code"}>
              <HighlightCode code={CardThumbnailTwoCode} />
            </Tab.Pane>
          </CustomTab>
        </div>
      </Col>
